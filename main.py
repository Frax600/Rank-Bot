...
async def get_rluser(username):
  ...
  response = await client.get('https://api.tracker.gg/api/v2/rocket-league/standard/profile/steam/' + username)
  try:
    json_data = json.loads(response.text)
  except:
  ...
  try:
    rank1v1 = json_data["data"]["segments"][1]["stats"]["tier"]["metadata"]["name"]
    div1v1 = json_data["data"]["segments"][1]["stats"]["division"]["metadata"]["name"]
    mmr1v1 = json_data["data"]["segments"][1]["stats"]["rating"]["value"]
    rank2v2 = json_data["data"]["segments"][2]["stats"]["tier"]["metadata"]["name"]
    div2v2 = json_data["data"]["segments"][2]["stats"]["division"]["metadata"]["name"]
    mmr2v2 = json_data["data"]["segments"][2]["stats"]["rating"]["value"]
    rank3v3 = json_data["data"]["segments"][3]["stats"]["tier"]["metadata"]["name"]
    div3v3 = json_data["data"]["segments"][3]["stats"]["division"]["metadata"]["name"]
    mmr3v3 = json_data["data"]["segments"][3]["stats"]["rating"]["value"]
...
async def on_message(message):
  ...
  if msg.startswith("!ranks(") and msg.endswith(")"):
    ...
    ranks = await get_rluser(username)
